...
test('acepta API mismo host o proxy mismo origen confirmado', () => {
  ...
  assert.deepEqual(validateCommerceConfig({ ...base, STORE_SAME_ORIGIN_PROXY_CONFIRMED: 'true' }), []);
});

test('rechaza build sin topología y API de otro host', () => {
  const base = { NODE_ENV: 'production', NEXT_PUBLIC_BASE_URL: 'https://teamcelular.com' };
  assert.equal(validateCommerceConfig(base).length, 1);
  assert.match(
