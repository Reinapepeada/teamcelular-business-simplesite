...
export async function generateMetadata({
  ...
  const hasFilters = Object.entries(resolvedSearchParams).some(
    ([key, value]) => key !== "page" && hasValue(value)
  );
  const shouldNoIndex = hasFilters || safePage > 1;
    ...
    <div className="w-full">
      <div className="max-w-screen-2xl mx-auto px-6 pt-6">
        <nav className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
          ...
          <span>/</span>
          <span className="text-slate-900 dark:text-white font-semibold">Tienda</span>
        </nav>
      </div>
