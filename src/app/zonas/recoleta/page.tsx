...
const ZONE_NAME = "Recoleta";
const PAGE_URL = `${SITE_URL}/zonas/recoleta`;

const highlights = [
  {
    title: "Taller en Recoleta",
    desc: "Estamos en Paraguay 2451. Podés venir directo o coordinar por WhatsApp para ahorrar tiempo.",
  },
  {
    title: "Arreglos frecuentes (en el día)",
    desc: "Pantalla, batería y problemas de carga suelen resolverse en el día según el modelo y stock.",
  },
  {
    title: "Microelectrónica (placa)",
    desc: "Fallas complejas: no enciende, reinicios, mojado o corto. Diagnóstico con microscopio y herramientas profesionales.",
  },
];

const localNotes = [
  "Si tu equipo se mojó, apagalo y no lo cargues: traelo cuanto antes para evitar corrosión.",
  "Si el touch anda mal o hay líneas/manchas, suele ser módulo display (no solo el vidrio).",
  "Si carga intermitente, puede ser pin sucio/desgastado o flex: lo confirmamos con diagnóstico.",
];

const faqs = [
  {
    q: `¿Dónde queda el taller en ${ZONE_NAME}?`,
    a: "Paraguay 2451, Recoleta (CABA). Atendemos de lunes a viernes de 10:30 a 18:00 hs. En la página de contacto tenés mapa y cómo llegar.",
  },
  {
    q: "¿Hacen arreglos de celulares en el día?",
    a: "En muchos casos sí (pantalla, batería o pin de carga). Si requiere diagnóstico de placa o repuesto especial, te informamos tiempos estimados antes de avanzar.",
  },
  {
    q: "¿Dan garantía por escrito?",
    a: "Sí. Entregamos garantía por escrito. El plazo depende del tipo de reparación y del repuesto utilizado; queda detallado en el presupuesto.",
  },
  {
    q: "¿Cómo pido presupuesto rápido?",
    a: "Podés completar el formulario o escribirnos por WhatsApp con marca, modelo y falla (si podés, sumá fotos).",
  },
];
...
export const metadata: Metadata = {
  ...
  description:
    "Arreglo de celulares en Recoleta, CABA: pantalla, batería, pin de carga, cámaras, audio y microelectrónica (placa). Taller en Paraguay 2451 con garantía por escrito y presupuesto rápido.",
...
export default function ZonaRecoletaPage() {
  const whatsappUrl = `https://wa.me/5491151034595?text=${encodeURIComponent(
    `Hola! Quiero un presupuesto para arreglo de celular en ${ZONE_NAME}. Marca y modelo:`
  )}`;
    ...
    <section className="w-full max-w-6xl px-6 py-14 md:px-8">
      ...
      <header className="rounded-2xl border border-white/15 bg-white/5 p-10 text-center shadow-lg backdrop-blur-2xl dark:border-white/10 dark:bg-slate-900/30 md:p-12">
        <h1 className="text-4xl font-bold tracking-tight text-slate-900 dark:text-white md:text-5xl">
          Arreglo de celulares en {ZONE_NAME} (CABA)
        </h1>
        <p className="mx-auto mt-5 max-w-3xl text-lg leading-relaxed text-slate-600 dark:text-slate-300">
          Si estás en {ZONE_NAME} y buscás <strong>arreglo / reparación</strong>{" "}
          de celulares, te atendemos en nuestro taller. Trabajamos pantalla,
      ...
      <section className="mt-10 rounded-2xl border border-white/15 bg-white/5 p-8 shadow-lg backdrop-blur-2xl dark:border-white/10 dark:bg-slate-900/30">
        ...
        <p className="mt-5 text-slate-600 dark:text-slate-300">
          Si no estás seguro qué es, pedí diagnóstico y te orientamos antes de
          avanzar.
        </p>
      </section>
